...
class resume extends Page {
    ...
    //untuk update resume - pendidikan
    get jumlahData () {
        return $$('.overflow-auto.no-scrollbar.gap-6.flex.flex-col > .card.bg-base-100.shadow.w-full').length;
    }
    get formTingkatPendidikan () {
        return $('select[name="jenjang-0"]');
    }
    get formTanggalMulai () {
        return $('input[name="tanggalMulai-0"]');
    }
    get formTanggalSelesai () {
        return $('input[name="tanggalSelesai-0"]');
    }
    get formNamaInstitusiPendidikan () {
        return $('input[name="nama-0"]');
    }
    get formJurusanPendidikan () {
        return $('input[name="jurusan-0"]');
    }
    get formNilaiAkhir () {
        return $('input[name="nilai-0"]');
    }
    get selectOptionReact () {
        return $$('.react-select__option');
    }
    /**
     ...
     */
    //general
    async switchToCurrentWindow() {
        await browser.waitUntil(async () => {
            const handles = await browser.getWindowHandles();
            return handles.length > 1;
        }, {
            timeout: 5000,
            timeoutMsg: 'Expected new tab to be opened'
        })
    ...
    //untuk update resume - pendidikan
    async waitJumlahData(){
        await browser.waitUntil(
            async () => {
                const jumlahDataPendidikanTime = await this.jumlahData
                return jumlahDataPendidikanTime > 0; 
            },
            {
                timeout: 3000,
                timeoutMsg: 'Jumlah data tidak berubah setelah 3 detik'
            }
        )
    }
    async inputTingkatPendidikan(n){
        await this.formTingkatPendidikan.selectByIndex(n)
    }
    async inputTanggalMulaiPendidikan(date){
        await this.formTanggalMulai.setValue(date)
    }
    async inputTanggalSelesaiPendidikan(date){
        await this.formTanggalSelesai.setValue(date)
    }
    async inputNamaInstitusiPendidikan(nama){
        await this.formNamaInstitusiPendidikan.setValue(nama)
    }
    async inputJurusanPendidikan(jurusan){
        await this.formJurusanPendidikan.setValue(jurusan)
    }
    async inputNilaiPendidikan(nilai){
        await this.formNilaiAkhir.setValue(nilai)
    }
    async inputBulanMulaiPendidikan(n,bulan){
        const formBulanMulaiPendidikan = await $(`#react-select-${n}-input`);
        await formBulanMulaiPendidikan.click();
        await this.selectOptionReact[bulan].click();
    }
    async inputBulanSelesaiPendidikan(n,bulan){
        const formBulanSelesaiPendidikan = await $(`#react-select-${n}-input`);
        await formBulanSelesaiPendidikan.click();
        await this.selectOptionReact[bulan].click();
    }
    /**
