...
class Confirm extends Page {
    ...
    get getTingkatPendidikan () {
        return $('select[name="jenjang-0"]');
    }
    get lengthTngkatPendidikan () {
        const selectElement = $('select[name="jenjang-0"]');
        const options = selectElement.$$('option');
        return options.length;
    }
    get getTahunMulaiPendidikan () {
        return $('input[name="tanggalMulai-0"]');
    }
    get getTahunSelesaiPendidikan () {
        return $('input[name="tanggalSelesai-0"]');
    }
    get getNamaInstitusiPendidikan () {
        return $('input[name="nama-0"]');
    }
    get getJurusanPendidikan () {
        return $('input[name="jurusan-0"]');
    }
    get getNilaiAkhirPendidikan () {
        return $('input[name="nilai-0"]');
    }
    get getTextBulan () {
        return $$('.react-select__single-value')
    }
    ...
    //untuk pendidikan
    async checkValidityTingkatPendidikan(){
        const element = await this.getTingkatPendidikan
        const isValid = await browser.execute((el) => {
            return el.checkValidity()
        }, element)
        return isValid;
    }
    async getTextTingkatPendidikan() {
        return await this.getTingkatPendidikan.getValue()
    }
    async checkValidityTahunMulaiPendidikan(){
        const element = await this.getTahunSelesaiPendidikan
        const isValid = await browser.execute((el) => {
            return el.checkValidity()
        }, element)
        return isValid;
    }
    async checkValidityTahunSelesaiPendidikan(){
        const element = await this.getTahunSelesaiPendidikan
        const isValid = await browser.execute((el) => {
            return el.checkValidity()
        }, element)
    ...
    async getValueTanggalMulaiPendidikan() {
        return await this.getTahunMulaiPendidikan.getValue()
    }
    async getValueTanggalSelesaiPendidikan() {
        return await this.getTahunSelesaiPendidikan.getValue()
    }
    async getValueNamaInstitusiPendidikan() {
        return await this.getNamaInstitusiPendidikan.getValue()
    }
    async getValueJurusanPendidikan() {
        return await this.getJurusanPendidikan.getValue()
    }
    async getValueNilaiPendidikan() {
        return await this.getNilaiAkhirPendidikan.getValue()
    }
    // async getBulanPendidikan (n) {
    //     return this.getTextBulan[n].getText();
    // }
}
